...
public class TencentFileStorage implements IFileStorage {
    ...
    public TencentFileStorage(COSClient tencentCosClient, TransferManager transferManager, TencentProperties properties) {
        ...
        this.publicDomain = getPublicDomain(properties);
    }
    ...
    private String getPublicDomain(TencentProperties properties) {
        String domain = properties.getCos().getDomain();
        if (domain == null || domain.trim().isEmpty()) {
            return "https://" + bucketName + ".cos." + properties.getCos().getRegion() + ".myqcloud.com";
        }
        domain = domain.trim();
        if (domain.endsWith("/")) {
