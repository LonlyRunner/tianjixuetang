...
public class ExamRecordController {
    ...
    @PostMapping
    public ExamQuestionVO saveExamRecord(@Valid @RequestBody ExamFormDTO examFormDTO){
        return examService.saveExamRecord(examFormDTO);
    }

    @ApiOperation("提交考试答案，考试或测试提交时需要保存答案信息")
    @PostMapping("/details")
    public void saveExamRecordDetails(@Valid @RequestBody ExamCommitDTO examCommitDTO){
