...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements IQuestionService {
    ...
    public PageDTO<QuestionPageVO> queryQuestionByPage(QuestionPageQuery query) {
        // 1.分页搜索
        normalizeQuestionQuery(query);
        Page<Question> page = baseMapper.queryQuestionPage(
    ...
    private void normalizeQuestionQuery(QuestionPageQuery query) {
        query.setCurrentUser(UserContext.getUser());
        String sortBy = query.getSortBy();
        if (StringUtils.isBlank(sortBy)) {
            query.setSortBy(Constant.DATA_FIELD_NAME_UPDATE_TIME);
        } else if ("useTimes".equals(sortBy)) {
            query.setSortBy("use_times");
        } else if ("answerTimes".equals(sortBy)) {
            query.setSortBy("answer_times");
        } else if ("updateTime".equals(sortBy)) {
            query.setSortBy(Constant.DATA_FIELD_NAME_UPDATE_TIME);
        }
        Set<String> allowedSortFields = Set.of(
                Constant.DATA_FIELD_NAME_UPDATE_TIME, "use_times", "answer_times");
        if (!allowedSortFields.contains(query.getSortBy())) {
            throw new BadRequestException("不支持的排序字段");
        }
        if (CollUtils.isNotEmpty(query.getTypes())) {
            List<Integer> supportedTypes = query.getTypes().stream()
                    .filter(t -> t != null && t != 5)
                    .collect(Collectors.toList());
            query.setTypes(CollUtils.isEmpty(supportedTypes) ? CollUtils.singletonList(-1) : supportedTypes);
        }
    }
    ...
    @Override
    @Transactional
    public void increaseAnswerStatistics(Map<Long, Integer> answerTimes, Map<Long, Integer> correctTimes) {
        if (CollUtils.isEmpty(answerTimes)) {
            return;
        }
        answerTimes.forEach((id, count) -> lambdaUpdate()
                .eq(Question::getId, id)
                .setSql("answer_times = answer_times + " + count)
                .setSql("correct_times = correct_times + " + correctTimes.getOrDefault(id, 0))
                .update());
    }
}
