...
public class ExchangeCodeServiceImpl extends ServiceImpl<ExchangeCodeMapper, ExchangeCode> implements IExchangeCodeService {
    ...
    public PageDTO<CodeVO> queryCodePage(CodeQuery query) {
        ...
        Page<ExchangeCode> page = lambdaQuery()
                ...
                .eq(status != null, ExchangeCode::getStatus, ExchangeCodeStatus.of(status))
                .page(query.toMpPageDefaultSortByCreateTimeDesc());

        return PageDTO.of(page, CodeVO.class);
    }

    @Override
    public boolean updateExchangeMark(long serialNum, boolean mark) {
        Boolean boo = redisTemplate.opsForValue().setBit(COUPON_CODE_USED_KEY, serialNum, mark);
        return boo != null && boo;
    }
